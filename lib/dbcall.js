...
DBCall.prototype.run = function(paramData, resultData, fields, callback) {
    ...
    if (this.config.dbcall === 'save') {
        ...
        for (var i = 0; i < keysLength; i++) {
            ...
            if ('findOneAndUpdate' === command) {
                emitSaveEvent = true;
            }
            if (/(findOneAndUpdate|update|findAndUpdate)/.test(command) && this.model.schema.methods.index) {
                updateCall = command;
            }
        ...
        if (!skipExec) {
            query.slaveOk();
            query.read(this.config.prefer || 'secondaryPreferred');
