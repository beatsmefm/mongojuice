...
var funks = {
    map: {
        ...
        handler: function(matches, data) {
            ...
            var results = [],
                data = matches[1] === 'result' ? this.paramData : this.determineValue('@' + matches[1], '@');
...
ParamParser.prototype.parse = function(){
    ...
    if('string' === typeof this.params) {
        ...
        _.each(this.params, function(paramExpression, paramName){
            args[paramName] = this._parse(paramExpression);
        }, this);
        return args;
    }
...
ParamParser.prototype.determineValue = function(expression, splitChar, isResult){
    ...
    if(dotIndex >= 0) {
        var fields = objName.split('.'),
            ...
            baseObj = _.extend({}, this.paramData, this.resultData),
            i = isResult  ? 1 : 0;
        ...
        obj = this.paramData[objName] || this.resultData[objName];
    }
    return obj;
}
