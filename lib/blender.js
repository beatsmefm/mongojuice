...
Blender.prototype.blend = function(key, params, middleware, callback){
    if(key.indexOf('db.') === 0) {
        ...
        var result = {};
        this._blend(this.blenderCalls, params, result, middleware, function(err){
            if(_.isEmpty(result)){
                result = null;
            }
...
Blender.prototype.makeCall = function(params, parent, blenderCall, middleware, callback) {
    ...
    async.waterfall([
        ...
        _.bind(function(data, next) {
            ...
            var callParams = new ParamParser(blenderCall.params, _.extend({}, data || {}, params)).parse();
            if(!next && 'function' === typeof data){
                next = data;
            }
            blenderCall.db.run(callParams, blenderCall.fields, _.bind(function(err, callResult){
        ...
        if(blenderCall.emits) {
            _.each(blenderCall.emits, function(emit){
                debugger;
                var data = new ParamParser(emit.params, _.extend({}, parent || {}, params)).parse();
