...
var Blender = function(key, config, dbCallManager) {
    ...
    if (this.dbCallManager.logger) {
        logger = this.dbCallManager.logger;
    }
    this.build();
...
Blender.prototype.checkForMissingParams = function(params) {
    'use strict';
...
Blender.prototype.blend = function(key, params, middleware, callback) {
    ...
    if (key.indexOf('db.') === 0) {
        ...
        var missing = this.checkForMissingParams(params);
        if (missing) {
...
function mainCall(params, parent, blenderCall, data, next) {
    ...
    blenderCall.db.run(callParams, data, blenderCall.fields, _.bind(function(err, callResult) {
        if (blenderCall.debug) {
            logger.info({
                source: 'mongojuice:blender:mainCall',
                message: 'blenderCall',
                name: blenderCall.name,
                result: util.inspect(callResult)
            });
        }
        next(err, callResult);
    }, this));
...
Blender.prototype.makeCall = function(params, parent, blenderCall, middleware, callback) {
    'use strict';
    var middle = new ParamParser({
        after: blenderCall.after,
        before: blenderCall.before
    }, middleware).parse();
    ...
    return async.waterfall([
        ...
        if (emits) {
            ...
            for (var i = 0; i < emitsLength; i++) {
                ...
                logger.info({
                    source: 'mongojuice:blender:makeCall',
                    message: 'emitting',
                    name: emit.name,
                    params: util.inspect(data)
                });
...
function applyChildResult(config, parentResult, childResult) {
    'use strict';
    var matches = 0,
...
function applyParentResult(config, result, parentResult) {
    'use strict';
    _.each(result, _.bind(applyChildResult, null, config, parentResult));
}
...
Blender.prototype.applyResult = function(config, parent, result) {
    ...
    if (!this.normalize && util.isArray(parent)) {
        ...
        _.each(parent, _.bind(applyParentResult, null, config, result));
    } else {
...
Blender.prototype.build = function() {
    ...
    _.each(this.callConfigs, function(config) {
        this.blenderCalls.push(new BlenderCall(_.extend({
            debug: this.debug
        }, config), this.dbCallManager));
    }, this);
};
