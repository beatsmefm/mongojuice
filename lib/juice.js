...
Juicer.prototype.init = function(config, callback) {
    ...
    var mongoUrl = "",
        db = config.db;
    if(db.replica_name && db.host) {
        mongoUrl = db.host;
    } else {
        if(!db.replica_name && db.username && db.password) {
            mongoUrl += db.username + ':' + db.password + '@';
        }
        mongoUrl += db.host + ':' + db.port + '/' + db.name;
    }
    logger.info("MONGOJUICE: Trying to connect to mongodb: %s", mongoUrl);
...
Juicer.prototype.load = function(){
    ...
    _.each(['db', 'blenders'], function(type){
        var files = fs.readdirSync(this.configDir + '/' + type);
